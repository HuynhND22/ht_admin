...
const Home: React.FC = () => {
  ...
  const getData = async () => {
    try {
      ...
      const response = await axiosClient.get(
        `/statistics/orders/date?from=${from}&to=${to}`,
        {
          headers: {
      ...
      const res = await axiosClient.get(
        `/statistics/status-orders/date?from=${from}&to=${to}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("accessToken")}`,
          },
  ...
  const labels: any[] = orders.map((x: any) =>
      ...
      <Row gutter={24}>
        ...
        <Col span={12}>
          <Card
            ...
            style={{ width: "100%", paddingLeft: "23%", paddingRight: "23%" }}
          >
