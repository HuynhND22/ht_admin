  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: string, record: any, index: number) => {
        return <Image src={process.env.REACT_APP_API_BASE_URL +'/' + record.cover} fallback={process.env.REACT_APP_API_BASE_URL + '/ImgError.png'} />
      },
    },
    {
      title: 'Danh mục',
            ...
            <Tooltip title="Chỉnh sửa">
              <Button
                ...
                onClick={() => {
                  ...
                  if(record.cover) {setFileListUpdate([
                    {
                      uid: record.cover?.split('/').pop(),
                      name: record.cover?.split('-').pop(),
                      status: 'done',
                      url: process.env.REACT_APP_API_BASE_URL + '/' + record.cover
                    }])}
              ...
              <Tooltip title="Xoá">
                <Button type='primary' danger icon={<DeleteOutlined />} onClick={()=>handleRemove(record.postId)} />
              </Tooltip>
            </Space>
          );
        } else {
  ...
  return (
    <div style={{ padding: 36 }}>
      <Card title='Tạo bài đăng mới' style={{ width: '100%' }}>
        <Form form={createForm} name='create-post' labelCol={{ span: 5 }} wrapperCol={{ span: 12 }} onFinish={onFinish}>
          <Form.Item label='Tên bài đăng' name='name' rules={[
          ...
          <Form.Item name='categoryId' label='Danh mục' rules={[{ required: true, message: 'Danh mục là bắt buộc!'}]} hasFeedback>
            <Select>
            {
          ...
          <Form.Item name="images" label="Ảnh bìa"
      ...
      >
        <Form form={updateForm} name='update-post' labelCol={{ span: 2 }} wrapperCol={{ span: 24 }} onFinish={onUpdate}>
          <Form.Item label='Tên bài đăng' name='name' rules={[
              { required: true, message: 'Tên bài đăng là bắt buộc!' },
              {
                validator(rule, value, callback) {
